...
Exercises the runtime under concurrent load using GTest, driven the way an embedder
drives it. One runtime per process takes liblogos' own `modules/` as its bundled
directory (`LOGOS_BUNDLED_MODULES_DIR`), so capability_module is its token authority,
and every load, unload and query is a `core_service` call over the shell binding of
`thread_safety_tests`. Only `logos_core_process_module` stays on the C API: it
belongs to the embedder alone.

The `test_dummy_module` binary is used as a template: each test generates 100 copies
by patching the embedded plugin name at the binary level, giving each thread a distinct
real Qt plugin to work with. The runtime outlives each test, so each test's copies
have names of their own. Tests cover:

- **Process** — `logos_core_process_module` concurrently on disjoint and shared module sets
- **Query under writes** — `listModules` (`all` / `loaded`) called by reader threads while writers are processing or loading
- **Load** — `loadModule(name, deps)` on disjoint and shared sets, with both `module_only` and `required`, including unknown-name fast-failure paths
- **Unload** — `unloadModule(name, withDependents = false)` interleaved with concurrent load threads on a shared small module set
